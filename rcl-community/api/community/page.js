...
//   /apps/community/profil/<handle>   -> ProfilePage + Person semasi
//   /apps/community/foto/<id>         -> ImageObject + creator semasi
//   /apps/community/yarisma/<slug>    -> Event semasi + en cok begenilenler
...
export default async function handler(req, res) {
  ...
  if (type === 'profil') {
    ...
    const url = SITE + '/apps/community/profil/' + pr.handle;
    ...
    const grid = (photos || []).map(function(p){return '<a href="'+SITE+'/apps/community/foto/'+p.id+'"><img loading="lazy" src="'+esc(p.image_url)+'" alt="'+esc(p.camera_model+', '+p.location)+'" width="240" style="width:100%;border-radius:12px"></a>'}).join('');
  ...
  if (type === 'foto') {
    ...
    const url = SITE + '/apps/community/foto/' + p.id;
    ...
    const jsonld = ld({ '@context':'https://schema.org','@type':'ImageObject',
      ...
      creator:{ '@type':'Person', name: author.display_name, url: SITE+'/apps/community/profil/'+author.handle },
    ...
    const body = '<h1>'+esc(p.camera_model)+' &#183; '+esc(p.location)+'</h1>'+
      ...
      '<p>Ceken: <a href="'+SITE+'/apps/community/profil/'+esc(author.handle)+'">@'+esc(author.handle)+'</a> &#183; '+esc(p.like_count)+' begeni</p>';
  ...
  if (type === 'yarisma') {
    ...
    const url = SITE + '/apps/community/yarisma/' + c.slug;
    ...
    const grid = (top||[]).map(function(p){return '<a href="'+SITE+'/apps/community/foto/'+p.id+'"><img loading="lazy" src="'+esc(p.image_url)+'" alt="'+esc(p.camera_model)+'" style="width:100%;border-radius:12px"></a>'}).join('');
