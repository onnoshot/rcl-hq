...
export async function get(req, res) {
  ...
  let pq = db.from('camera_photos')
    .select('id,image_url,location,taken_date,like_count,visibility,cameras(brand,model)')
    .eq('customer_id', pr.customer_id).eq('status', 'approved');
  if (!own) pq = pq.eq('visibility', 'public');   // baskasinin profilinde sadece herkese acik anilar
  const { data: photos } = await pq.order('created_at', { ascending: false }).limit(60);

  // begendigi (oyladigi) kameralar + bekleme listesi (tum profillerde; gorunurlugu section_prefs belirler)
  ...
  return res.status(200).json({
    ...
    profile: { handle: pr.handle, display_name: pr.display_name, avatar_url: pr.avatar_url, bio: pr.bio,
      ...
      links: pr.links || {}, section_prefs: pr.section_prefs || {} },
    cameras: cams || [],
...
export async function update(req, res) {
  ...
  // sosyal linkler (whitelist anahtar; deger handle veya URL, temizlenir)
  if (b.links && typeof b.links === 'object') {
    const KEYS = ['instagram', 'threads', 'x', 'facebook', 'pinterest', 'website'];
    const links = {};
    for (const k of KEYS) {
      const v = b.links[k];
      if (typeof v === 'string' && v.trim()) {
        links[k] = v.trim().replace(/\s+/g, '').replace(/[<>"']/g, '').slice(0, 120);
      }
    }
    patch.links = links;
  }

  // bolum gorunurluk tercihleri (ziyaretciye gizle): {badges,liked,wait,cams,photos:false}
  if (b.section_prefs && typeof b.section_prefs === 'object') {
    const SK = ['badges', 'liked', 'wait', 'cams', 'photos'];
    const sp = {};
    for (const k of SK) if (b.section_prefs[k] === false) sp[k] = false;
    patch.section_prefs = sp;
  }
  ...
  const { data: pr, error } = await db.from('profiles').update(patch).eq('customer_id', claims.sub)
    .select('handle,display_name,avatar_url,city,links,section_prefs').single();
