...
export async function run(req, res) {
  ...
  // ONEMLI: zaten OZGUN aciklamasi olan kameralarda description'i EZME (magaza body_html'i jenerik sablondur).
  // Sadece aciklamasi bos olan (yeni) urunlere body_html'den baslangic aciklamasi koy.
  const { data: ex } = await db.from('cameras').select('handle,description');
  const hasDesc = new Set((ex || []).filter((c) => c.description && String(c.description).trim()).map((c) => c.handle));
  rows.forEach((r) => { if (hasDesc.has(r.handle)) delete r.description; });
