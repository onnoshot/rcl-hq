// FAZ 2: GET /apps/hesabim/community/sitemap  -> topluluk URL'leri icin XML sitemap (Shopify bunlari uretmez)
// Search Console'a https://retrocameraland.com/apps/hesabim/community/sitemap olarak ekle.
...
export async function run(req, res) {
  ...
  (contests || []).forEach(function(c){ urls.push([SITE+'/apps/hesabim/community/yarisma/'+c.slug, c.created_at]); });
  ...
  (photos || []).forEach(function(p){
    urls.push([SITE+'/apps/hesabim/community/foto/'+p.id, p.created_at]);
    const h = p.profiles && p.profiles.handle; if (!h) return;
    perUser[h] = perUser[h] || { n:0, mod: p.profiles.updated_at };
    perUser[h].n++;
  });
  Object.keys(perUser).forEach(function(h){ if (perUser[h].n >= 2) urls.push([SITE+'/apps/hesabim/community/profil/'+h, perUser[h].mod]); });
