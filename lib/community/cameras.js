...
export async function list(req, res) {
  ...
  let q = db.from('cameras')
    .select('id,handle,brand,model,title,image_url,price,product_url,score,votes_count,photo_count')
    .order('score', { ascending: false }).range(from, from + PAGE - 1);
...
export async function detail(req, res) {
  ...
  return res.status(200).json({ ok: true, camera: { ...cam, voted }, photos: ph });
}
