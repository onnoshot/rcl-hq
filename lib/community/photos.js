...
export async function like(req, res) {
  ...
  const db = adminClient();
  if (b.action === 'unlike') {
    await db.from('photo_likes').delete().eq('photo_id', b.photo_id).eq('customer_id', claims.sub);
  } else {
    await db.from('photo_likes').upsert({ photo_id: b.photo_id, customer_id: claims.sub }, { onConflict: 'photo_id,customer_id', ignoreDuplicates: true });
  }
